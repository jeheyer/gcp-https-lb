...
resource "google_compute_target_tcp_proxy" "default" {
  ...
  project         = var.project_id
  name            = "${local.name}-${lower(local.type)}"
...
resource "google_compute_target_https_proxy" "default" {
  ...
  ssl_certificates = local.use_ssc ? [google_compute_ssl_certificate.default["localhost"].name] : [
    for k, v in var.ssl_certificates : google_compute_ssl_certificate.default[k].id
  ]
...
resource "google_compute_region_target_https_proxy" "default" {
  ...
  ssl_certificates = local.use_ssc ? [google_compute_region_ssl_certificate.default["localhost"].name] : [
    for k, v in var.ssl_certificates : google_compute_ssl_certificate.default[k].id
  ]
  region = local.region
